...
export function useChat(sessionId: string, initialMessages: Message[]) {
  ...
  const runStream = useCallback(
    async ({ mode, model, request }: RunStreamsParams) => {
  ...
  // Auto-resume when the conversation ends with user message and no assistant response
  ...
  const submit = useCallback(
    async ({ userText, mode, model }: SubmitParams) => {
      // Shows the partial answer before sending the next message
